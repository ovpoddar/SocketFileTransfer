    ...
    public partial class SendForm : Form
    {
        ...
        private void Ping_PingCompleted(object sender, PingCompletedEventArgs e)
        {
            ...
            try
            {
                ...
                _streams[current].BeginRead(model.Buffer, 0, model.Buffer.Length, DataReceived, model);
        ...
        {
            var model = ar.AsyncState as Transfer;
            var received = _streams[model.Id].EndRead(ar);

            if (model.Client.ReceiveBufferSize < 0)
                return;

            var name = Encoding.ASCII.GetString(model.Buffer, 0, received);
